...
export function ChatShell() {
  ...
  // Entry animation: true when opening fullscreen from homepage prompt bar
  const [enteringFromHome, setEnteringFromHome] = useState(false);
  // Collapse animation: true when transitioning fullscreen → sidepanel on canvas creation
  const [collapsingToSidepanel, setCollapsingToSidepanel] = useState(false);
  ...
  // Check for homepage entry animation flag
  useLayoutEffect(() => {
  ...
  // Check for collapse-to-sidepanel animation flag
  useEffect(() => {
    if (!isFullscreen) return;
    const shouldAnimate = sessionStorage.getItem("chat-animate-to-sidepanel");
    if (shouldAnimate === "true") {
      sessionStorage.removeItem("chat-animate-to-sidepanel");
      setCollapsingToSidepanel(true);
    }
  }, [isFullscreen]);
  ...
  // Fullscreen: fixed overlay with animated entry/collapse
  if (isFullscreen) {
    // Determine animation state
    const collapseTargetLeft = typeof window !== "undefined"
      ? window.innerWidth - panelWidth
      : 0;

    if (collapsingToSidepanel) {
      return (
        <motion.div
          key="collapsing"
          initial={{ left: appSidebarWidth, borderRadius: "0px" }}
          animate={{
            left: collapseTargetLeft,
            borderRadius: "24px 0px 0px 24px",
          }}
          transition={spring.default}
          onAnimationComplete={() => {
            setCollapsingToSidepanel(false);
    ...
    return (
      <motion.div
        key="fullscreen"
        initial={enteringFromHome ? { y: "60vh", opacity: 0 } : false}
        animate={{ y: 0, opacity: 1 }}
        transition={spring.snappy}
        onAnimationComplete={() => {
          if (enteringFromHome) setEnteringFromHome(false);
        }}
        style={{
          position: "fixed",
          top: 0,
