...
import { IconButton as MdsIconButton, Tooltip, Button, Text } from "@mirohq/design-system";
...
import { CATEGORIES, type Category } from "@/data/guidedCategories";
...
function QuestionBlock({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <p style={{ fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>{question}</p>
      <div
        style={{
          display: "flex",
      ...
      >
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {suggestions.map((suggestion, i) => (
            <button
              key={i}
              onClick={() => !isAnswered && onSelect(suggestion)}
              disabled={isAnswered}
              ...
              style={{
                ...
                padding: "10px 16px",
                borderRadius: 9999,
                border: "1px solid var(--color-gray-200, #E7E7E5)",
                background: "var(--color-gray-50, #FBFAF7)",
                ...
                lineHeight: 1.4,
                fontWeight: 400,
                color: "var(--color-gray-900, #191812)",
              }}
              onMouseEnter={(e) => { if (!isAnswered) { (e.currentTarget as HTMLElement).style.background = "var(--color-gray-100, #F4F4F1)"; (e.currentTarget as HTMLElement).style.borderColor = "var(--color-gray-300, #D5D5D2)"; } }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "var(--color-gray-50, #FBFAF7)"; (e.currentTarget as HTMLElement).style.borderColor = "var(--color-gray-200, #E7E7E5)"; }}
            >
              {suggestion}
            </button>
          ))}
        </div>
        {isLatest && !isAnswered && onSkip && (
          <div style={{ display: "flex", justifyContent: "flex-end", width: "100%" }}>
            <Button variant="secondary" size="medium" onPress={onSkip}>
...
export function ChatPanel({
  ...
  const [askBoardActive, setAskBoardActive] = useState(false);
  const [activeCategory, setActiveCategory] = useState<Category | null>(null);

  // Reset only when a genuinely NEW askUser message appears (not on every message change)
  useEffect(() => {
    const latestAskUser = messages.findLast((m: Message) => m.toolInvocations?.some((t: any) => t.toolName === 'askUser'));
  ...
  const handleGuidedSubmit = useCallback((query: string) => {
    const match = CATEGORIES.find(c => c.label === query);
    if (match) setActiveCategory(match);
    onSubmit(query);
  }, [onSubmit]);

  const handleDismissCategory = useCallback(() => {
  ...
  // Reset askBoardActive when new messages arrive (new question cycle)
  useEffect(() => {
    if (askBoardActive) setAskBoardActive(false);
  ...
  const activeQuestion = useMemo(() => {
    ...
    if (askBoardActive) return null;
    const latestAskUser = messages.findLast((m: Message) =>
      m.toolInvocations?.some((t: any) => t.toolName === 'askUser')
    );
    if (!latestAskUser) return null;

    const askIdx = messages.indexOf(latestAskUser);
    const nextUserMsg = messages.slice(askIdx + 1).find((m: Message) => m.role === 'user');
    if (nextUserMsg) return null;

    const tool = latestAskUser.toolInvocations!.find((t: any) => t.toolName === 'askUser')!;
    const args = tool.args as { questions?: Array<{question: string; suggestions: string[]; displayType?: "options" | "scale" | "time" | "counter" | "multiChoice" | "matrix" | "stylePicker" | "colorPicker" | "peoplePicker"; leftLabel?: string; rightLabel?: string; topLabel?: string; bottomLabel?: string; scaleLeftColor?: string; scaleRightColor?: string; maxMinutes?: number; maxCount?: number; counterUnit?: string}>; question?: string; suggestions?: string[] };
    ...
    return {
      ...
      displayType: currentQ.displayType,
      leftLabel: currentQ.leftLabel,
      rightLabel: currentQ.rightLabel,
      topLabel: currentQ.topLabel,
      bottomLabel: currentQ.bottomLabel,
      scaleLeftColor: currentQ.scaleLeftColor,
      scaleRightColor: currentQ.scaleRightColor,
      maxMinutes: currentQ.maxMinutes,
      maxCount: currentQ.maxCount,
      counterUnit: currentQ.counterUnit,
      questionIndex: currentQuestionIndex,
      totalQuestions: questions.length,
      questions,
    };
  }, [messages, isFullscreen, currentQuestionIndex, questionAnswers, askBoardActive]);
  ...
  // Handler for "Ask the Board" — push question to canvas widget (or scale vote for scale questions)
  const handleAskBoard = useCallback(() => {
    if (!activeQuestion) return;
    if (activeQuestion.displayType === "scale") {
      window.dispatchEvent(
        new CustomEvent("scalevote:create", {
          detail: {
            question: activeQuestion.question,
            leftLabel: activeQuestion.leftLabel,
            rightLabel: activeQuestion.rightLabel,
          },
        })
      );
    } else {
      window.dispatchEvent(
        new CustomEvent("askboard:create", {
          detail: {
            question: activeQuestion.question,
  ...
  return (
    <div className="w-full h-full bg-white flex flex-col">
      {/* Header — matches Figma 84-16310 */}
      <div
        style={{
          display: "flex",
          alignItems: "center",
          gap: "var(--spacing-50, 4px)",
          paddingLeft: "var(--spacing-300, 24px)",
          paddingRight: "var(--spacing-150, 12px)",
          height: 56,
          flexShrink: 0,
      ...
      >
        <div style={{ flex: "1 0 0", display: "flex", alignItems: "center", minWidth: 0 }}>
          <Button variant="ghost" size="medium">
            <Button.Label>Admin sidekick</Button.Label>
            <Button.IconSlot><IconChevronDown /></Button.IconSlot>
          </Button>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "var(--spacing-50, 4px)", flexShrink: 0 }}>
          {isFullscreen && onExitFullscreen && (
            <Tooltip>
              <Tooltip.Trigger asChild>
                <MdsIconButton aria-label="Collapse to side panel" variant="ghost" size="medium" onPress={onExitFullscreen}>
                  <IconArrowsInSimple />
                </MdsIconButton>
              </Tooltip.Trigger>
          ...
          {!isFullscreen && (
            <Tooltip>
              <Tooltip.Trigger asChild>
                <MdsIconButton aria-label="Expand" variant="ghost" size="medium" onPress={onExpand}>
                  <IconArrowsOutSimple />
                </MdsIconButton>
              </Tooltip.Trigger>
          ...
          {!isFullscreen && onCollapse && (
            <Tooltip>
              <Tooltip.Trigger asChild>
                <MdsIconButton aria-label="Close sidebar" variant="ghost" size="medium" onPress={onCollapse}>
                  <IconViewSideRight />
                </MdsIconButton>
              </Tooltip.Trigger>
      ...
      {/* Messages */}
      <div className={`flex-1 overflow-y-auto px-6 pt-6 pb-28 ${isFullscreen ? 'mx-auto w-full max-w-3xl' : ''}`} style={{ fontFamily: "'Noto Sans', sans-serif", display: "flex", flexDirection: "column", gap: 24 }}>
        {messages.length === 0 ? (
          <GuidedStartPage onSubmit={handleGuidedSubmit} />
        ) : (
          messages.map((message, index) => {
            ...
            // Hide askUser responses — they're shown inline in the Q&A block
                ...
                {message.role === "user" ? (
                    <div
                      style={{
                        ...
                        borderRadius: 16,
                        padding: 12,
                        background: "var(--color-gray-100, #F4F4F1)",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                      }}
                    >
                      <p className="whitespace-pre-wrap" style={{ flex: "1 0 0", minWidth: 1, fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>{message.content}</p>
                    </div>
                ) : (
                  ...
                  <div className="w-full">
                    ...
                    {(() => {
                      ...
                      const mdComponents = {
                        p: ({ children }: { children?: React.ReactNode }) => <p style={{ marginBottom: 12 }} className="last:mb-0">{children}</p>,
                        strong: ({ children }: { children?: React.ReactNode }) => <strong className="font-semibold">{children}</strong>,
                        em: ({ children }: { children?: React.ReactNode }) => <em className="italic">{children}</em>,
                        ul: ({ children }: { children?: React.ReactNode }) => <ul className="list-disc pl-5" style={{ marginBottom: 12 }}>{children}</ul>,
                        ol: ({ children }: { children?: React.ReactNode }) => <ol className="list-decimal pl-5" style={{ marginBottom: 12 }}>{children}</ol>,
                        li: ({ children }: { children?: React.ReactNode }) => <li style={{ marginBottom: 4 }}>{children}</li>,
                        code: ({ children }: { children?: React.ReactNode }) => <code style={{ background: "var(--color-gray-100, #F4F4F1)" }} className="px-1 rounded text-xs">{children}</code>,
                      };
                      ...
                      return (
                        <>
                          ...
                          {ackText && (
                            <div style={{ fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>
                          ...
                          {summaryText && (
                            <div className="mt-4" style={{ fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>
                    ...
                    {askUserTool && (() => {
                      ...
                      if (nextUserMsg) {
                        // All answered — show faded suggestions + answer bubble
                              ...
                              <div key={qi} className={qi > 0 ? "mt-4" : ""}>
                                ...
                                {answers[qi] && (
                                  <div className="flex justify-end" style={{ marginTop: 8 }}>
                                    <div style={{ maxWidth: 260, borderRadius: 16, padding: 12, background: "var(--color-gray-100, #F4F4F1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                      <p style={{ flex: "1 0 0", minWidth: 1, fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>{answers[qi]}</p>
                                    </div>
                                  </div>
                                )}
                              </div>
                              ...
                              <div key={qi} className={qi > 0 ? "mt-4" : ""}>
                                {hasAnswer ? (
                                  <>
                                    ...
                                    <div className="flex justify-end" style={{ marginTop: 8 }}>
                                      <div style={{ maxWidth: 260, borderRadius: 16, padding: 12, background: "var(--color-gray-100, #F4F4F1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                        <p style={{ flex: "1 0 0", minWidth: 1, fontSize: 14, lineHeight: 1.4, color: 'var(--color-gray-900, #191812)', fontWeight: 400 }}>{questionAnswers[qi]}</p>
                                      </div>
                                    </div>
                                  </>
                                ) : (
                                  <QuestionBlock
                                    ...
                                    onSkip={() => {
                                      const newAnswers = [...questionAnswers];
                                      for (let si = qi; si < questions.length; si++) {
                                        if (newAnswers[si] === undefined) newAnswers.push("skip");
                                      }
                                      onSubmit(newAnswers.join("\n"));
                                    }}
...
{/* Single unified activity indicator - shows current activity */}
      ...
      {isFullscreen && activeQuestion && !isLoading && (
        <div className="flex justify-center px-4 pb-2 mx-auto w-full" style={{ maxWidth: 720 }}>
          <FloatingQuestionCard
            ...
            displayType={activeQuestion.displayType}
            leftLabel={activeQuestion.leftLabel}
            rightLabel={activeQuestion.rightLabel}
            topLabel={activeQuestion.topLabel}
            bottomLabel={activeQuestion.bottomLabel}
            scaleLeftColor={activeQuestion.scaleLeftColor}
            scaleRightColor={activeQuestion.scaleRightColor}
            maxMinutes={activeQuestion.maxMinutes}
            maxCount={activeQuestion.maxCount}
            counterUnit={activeQuestion.counterUnit}
      ...
      {/* Input — hidden when fullscreen plan approval is showing */}
      {!(isFullscreen && pendingApproval && !isLoading) && (
        <div style={{ position: "relative", paddingLeft: 24, paddingRight: 24, paddingTop: 8, paddingBottom: 24 }} className={isFullscreen ? 'mx-auto w-full max-w-3xl' : ''}>
          {isVoiceActive ? (
            <VoiceStopButton
            ...
            <ChatInput
              ...
              voiceState={voiceState}
              activeCategory={activeCategory ? { label: activeCategory.label, icon: activeCategory.icon } : null}
